...
fn list_enabled_flows_excludes_disabled() {
    ...
    let enabled_flow = create_flow(&config, "enabled".to_string(), trigger_graph()).unwrap();
    let disabled_flow = create_flow(&config, "disabled".to_string(), trigger_graph()).unwrap();
...
fn list_flow_runs_orders_newest_first_and_is_scoped_to_flow() {
    let tmp = TempDir::new().unwrap();
    let config = test_config(&tmp);
    let flow_a = create_flow(&config, "a".to_string(), trigger_graph()).unwrap();
    let flow_b = create_flow(&config, "b".to_string(), trigger_graph()).unwrap();
...
fn insert_duplicate_flow_makes_a_disabled_copy_with_new_id_and_same_graph() {
    ...
    // Enabled source with a distinctive graph name.
    let mut graph = trigger_graph();
    graph.name = "original-graph".to_string();
    let source = create_flow(&config, "My Flow".to_string(), graph).unwrap();
