...
| `media` | ON | `openhuman::media_generation` (the `media_generate_*` agent tools) | none (surface-only) |
...
**Leaf-gate variant (`media`, #4804).** Unlike `voice`, the `media` gate needs **no** stub facade: `media_generation` has a single caller (the `build_media_tools` call in `src/openhuman/tools/ops.rs`, itself `#[cfg(feature = "media")]`), so the module is simply `#[cfg(feature = "media")] pub mod media_generation;`. (The former `openhuman::image` unwired scaffold from #2997 was deleted in #5056 — dead, 0 callers, dropped no deps.) It is a **surface-only** gate: media generation is backend-proxied (`reqwest`, shared) and the `image` crate is shared with channel upload, so no exclusive deps are shed — the issue's "sheds media processing dependencies" / "controllers unregistered" DoD lines are superseded (Media is agent-tools-only; no controller/store/subscriber is tagged `Media`). When a gated domain is a true leaf, prefer this over the facade+stub.
